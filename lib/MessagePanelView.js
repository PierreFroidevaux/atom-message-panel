  ...
  this.div({
    ...
    this.div({
      ...
      this.div({
        ...
        this.div({
          id: 'btnError',
          class: 'heading-error inline-block icon-grabber',
          style: 'cursor: pointer',
          outlet: 'btnError',
          click: 'toggleError',
          title: 'No filter'
...
MessagePanelView.prototype.toggleError = function () {
  this.updateError();
};

MessagePanelView.prototype.updateError = function () {
  var els = document.querySelectorAll('.message'), i;
  if (this.btnError.hasClass('icon-grabber')) {
    this.btnError.toggleClass('icon-grabber');
    this.btnError.toggleClass('icon-italic');
    document.getElementById('btnError').title = 'Low Severity';
    els.forEach((item, i) => {
      if (item.classList.contains("LOW")) {
        void(item.parentElement.classList.contains("hidden") && item.parentElement.classList.remove("hidden"));
      } else {
        item.parentElement.classList.add('hidden')
      }
    });
  } else if (this.btnError.hasClass('icon-italic')) {
    this.btnError.toggleClass('icon-italic');
    this.btnError.toggleClass('icon-info');
    document.getElementById('btnError').title = 'Medium Severity';
    els.forEach((item, i) => {
  ...
  } else if (this.btnError.hasClass('icon-info')) {
    this.btnError.toggleClass('icon-info');
    this.btnError.toggleClass('icon-flame');
    document.getElementById('btnError').title = 'High Severity';
    els.forEach((item, i) => {
    ...
    this.btnError.toggleClass('icon-flame');
    this.btnError.toggleClass('icon-grabber');
    document.getElementById('btnError').title = 'No filter';
    els.forEach((item, i) => {
